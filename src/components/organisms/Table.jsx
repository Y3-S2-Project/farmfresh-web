...
export const Table = () => {
  return (
    <table className='w-1113'>
      <thead className="bg-white border-2 border-gray-700 shadow-md " style={{ width: '1113px', height: '70px', marginBottom: '10px' }}>
        <tr className="flex justify-center items-center " style={{ width: '1113px', height: '65px' }}>
          {TABLE_HEADER_CONTENT?.map((item, index) => (
            <th key={index} className="flex justify-center items-center" style={{ width: '1047px', height: '70px' }}>{item}</th>
          ))}
        </tr>
      </thead>
        ...
        {ADMIN_PRODUCT_TABLE_DATA?.map((row, index) => {
          ...
          cells = Object.values(row).map((data, cellIndex) => {
            if (Array.isArray(data) && data.every(item => typeof item === 'string' && item.startsWith('https'))) {
              return (
                <td key={cellIndex} style={{ width: '1047px', height: '70px', padding: '10px' }}>
                  <div className='flex'>
                    {data.map((item, index) => (
                      <img key={index} src={item} alt="im"  style={{ width: '55px', height: '55px' }} />
                    ))}
                  </div>
                </td>
              );
            ...
            return <td key={cellIndex}  className="flex justify-center items-center" style={{ width: '1047px', height: '70px', padding: '20px',paddingLeft:"30px" }}><Pill type={ data} /></td>;
          });

          return (
            <tr key={index} className='flex flex-row items-start gap-3' style={{ width: '1113px', height: '70px' }}>
              {cells}
